...
import os
os.environ['kaggle_username'] ="xxxxx" # replace with your own credential here temporarily or set up a secret scope with your credential

os.environ['kaggle_key'] = "xxxxx" # replace with your own credential here temporarily or set up a secret scope with your credential
...
# def copy_files(src, dest, file_pattern):
#   # create the destination directory if needed
#     makedirs(dest, exist_ok=True)
#     # create full paths for all files we wish to copy
#     src_paths = list(Path(src).rglob(file_pattern))
#     print("Total files to copy {}".format(len(src_paths)))
#     # process all file paths
#     for src_path in src_paths:
#         # copy source file to dest file
...
# Example
source_dir = "/local_disk0/ILSVRC/Annotations"
target_dir =  "/dbfs/tmp/dl/ILSVRC/Annotations"
...
# Example
source_dir = "/local_disk0/ILSVRC/Data"
target_dir =  "/dbfs/tmp/dl/ILSVRC/Data"
...
spark_write_path = "/tmp/dl/cv_datasets/imagenet"

# COMMAND ----------

from pyspark.sql import Window
from pyspark.sql.functions import rand, row_number, lit, col
...
# MAGIC %sql optimize delta.`/tmp/dl/cv_datasets/imagenet_train.delta` zorder by id

# COMMAND ----------

# MAGIC %sql optimize delta.`/tmp/dl/cv_datasets/imagenet_val.delta` zorder by id
